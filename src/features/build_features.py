import sys
sys.path.append("../modules")  # adjust path if needed
...
# --------------------------------------------------------------
predictor_columns_base = list(df.columns[:6])

for col in predictor_columns_base:
...
for s in df["set"].unique():
    ...
    df.loc[df["set"] == s, "duration"] = (stop - start).seconds
...
# Resultant acceleration + gyroscope
df_features["acc_r"] = np.sqrt(
    df_features["acc_x"]**2 +
    df_features["acc_y"]**2 +
    df_features["acc_z"]**2
)

df_features["gyr_r"] = np.sqrt(
    df_features["gyr_x"]**2 +
    df_features["gyr_y"]**2 +
    df_features["gyr_z"]**2
)

# FINAL predictor list (AFTER features exist)
predictor_columns = list(df_features.columns[:6]) + ["acc_r", "gyr_r"]

# --------------------------------------------------------------
# LOW PASS FILTER (FIXED PIPELINE)
# --------------------------------------------------------------
df_lowpass = df_features.copy()
...
# apply filter to all predictors
...
# --------------------------------------------------------------
# Signal magnitude features (SAFE NOW)
# --------------------------------------------------------------
...
# # Cluster with k=5, that's the elbow point in the plot
# kmeans = KMeans(n_clusters=5, n_init=20, random_state=0)
# subset = df_cluster[cluster_columns]
